...
def build_frequency_table(data: str):
    ...
    for ch, fr in freq.items():
        symbol = ch if ch != "\n" else "\\n"
        print(f"Символ '{symbol}' → частота {fr}")
...
def build_huffman_tree(freq_table):
    ...
    for ch, fr in freq_table.items():
        ...
        symbol = ch if ch != "\n" else "\\n"
        print(f"Додаємо вузол: '{symbol}' з частотою {fr}")

    print("Початковий heap:", heap)

    # Спецвипадок: один символ
    if len(heap) == 1:
        print("У файлі лише один унікальний символ — додаємо фейковий вузол.")
    ...
    while len(heap) > 1:
        ...
        print(f"Витягуємо 2 мінімальних вузли: {left} та {right}")
        ...
        print(f"Створюємо новий вузол частотою {merged.freq}")
        print("Поточний heap:", heap)
...
def bits_to_bytes(bitstring):
    ...
    print(f"\nДодаємо {padding} біт(ів) доповнення для вирівнювання до байта.")
...
def bytes_to_bits(padded_data):
    ...
    print(f"Зчитано {padding} біт доповнення, видаляємо їх.")
...
def huffman_compress(input_path, output_path):
    print("\n================ С Т И С Н Е Н Н Я ================")
    print(f"Завантажуємо файл: {input_path}")

    with open(input_path, "r", encoding="utf-8") as f:
        data = f.read()

    print(f"Довжина вхідного тексту: {len(data)} символів")
    ...
    print(f"Файл успішно стиснено → {output_path}")
...
def huffman_decompress(input_path, output_path):
    ...
    for _ in range(symbols_count):
        ...
        freq_table[ch] = freq
        symbol = ch if ch != "\n" else "\\n"
        print(f"Символ '{symbol}', частота {freq}")

    print("\nБудуємо дерево Хаффмана для декодування...")
    tree = build_huffman_tree(freq_table)

    # Решта — бітовий потік
    bit_data = bytes_to_bits(raw[pos:])

    print("\n=== КРОК 2: Декодування бітового потоку ===")
    result = []
    node = tree
